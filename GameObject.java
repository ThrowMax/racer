...
public class GameObject {
    ...
    public boolean isCollisionPossible(GameObject otherGameObject) {
        if (x-3 > otherGameObject.x + otherGameObject.width || x-3 + width < otherGameObject.x) {
            return false;
        }

        if (y-3 > otherGameObject.y + otherGameObject.height || y-3 + height < otherGameObject.y) {
            return false;
        }
        return true;
    }
